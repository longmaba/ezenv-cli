...
export class AuthService {
  ...
  async getStoredTokenData(): Promise<StoredTokenData | null> {
    const data = await this.credentialService.retrieve(this.getServiceName(), 'token_data')
  ...
  async getStoredToken(): Promise<string | null> {
    ...
    const needsRefresh = await this.isTokenExpired()
    if (needsRefresh) {
      const refreshed = await this.refreshToken()
      if (!refreshed) {
        return null
      }
      const freshData = await this.getStoredTokenData()
      return freshData?.access_token || null
    }
  ...
  async deleteStoredToken(): Promise<boolean> {
    return await this.credentialService.delete(this.getServiceName(), 'token_data')
  }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            errorMessage =
              parsedError.error_description || parsedError.msg || parsedError.error || errorMessage
